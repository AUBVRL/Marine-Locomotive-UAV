...
%load('U');
%load('P_gyro_Noisy');
file_name = ['Monte Carlo\CM_3',Environment,'\SVCS\']; % new
load_parameters()
...
% -------------------------------------------------------------------------
file_name = ['Monte Carlo\CM_4',Environment,'\SVCS\']; % new
load_parameters()
...
dz_u_max_SVC_l1 = max(abs(z_u_l1_SVC - z_u_bar_l1_SVC));
%max(abs(z_u_l2_PID - z_u_bar_l2_PID))
dz_u_max_SVC_l2 = max(abs(z_u_l2_SVC - z_u_bar_l2_SVC));
...
xlabel({'Time (s)';...
    ['                                                      ',...
    '(a) S3: medium sized multirotor with cable length of 15 m.']})
axis([0 t1-t0 0 500])
...
% print -dpdf -painters Plots/all_states_S3_E1_0
print(['Plots/all_states_S3_E',Environment,'_0'],'-dpdf')
print(['Plots/all_states_S3_E',Environment,'_0'],'-dpng','-r500')
...
set(gcf,...
    'PaperPosition',[0 0 screenposition(3:4)],...
    'PaperSize',[screenposition(3:4)]);
% print -dpdf -painters Plots/all_states_S4_E1_0
print(['Plots/Monte Carlo/all_states_S4_E',Environment,'_0'],'-dpdf')
print(['Plots/Monte Carlo/all_states_S4_E',Environment,'_0'],'-dpng','-r500')
