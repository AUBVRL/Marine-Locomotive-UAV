...
% Cuboid:
h_b = 0.3;            % buoy side (m), cuboid 
A_b = h_b^2;           % (m^2)
l_b = 1.5;             % buoy length (m)
...
%% Cable:
Lc_0 = 20;  % 10              % Cable free length (m)
dL_max = 0.0;            % maximum cable elongation (m)
T_max = 200;              % max cable tension (N)
...
%% quadrotor GAIA-160 hybrib:
m_u = 19.5 ;             % multirotor mass (kg)  1.634
J_u = 1.0*1.5*0.8^2+1*1.6^2/12+16.5*0.5^2/12;              % moment of inertia (kg.m2)
K = 500/6;               % motors maximum thrust,each (N) (6 motors)
n_motors = 6;            % number of motors
L = 0.8;                 % multirotor arm length (m)
if n_motors == 4
...
LIMIT_uCMD_HEIGHT = 0.9*n_motors*K;
LIMIT_CMD_x = 3;
...
Cd_u = 0.2;               
% Ac = [0.0331 0.0331 0.05]; % Cross section area of the multirotor in x and z direction (m^2)
Ac = 0.5*0.3+0.6*0.08*4+0.4*0.2;
...
% r_ref filter
sys=tf(1,conv(conv([0.7 1],[0.7 1]),conv([0.7 1],[0.7 1]))); % 0.5
sysd = c2d(sys,T);
...
V_max = (m_b + m_u - epsilon_2*rho_w)*g*tan(Theta_mean)...
